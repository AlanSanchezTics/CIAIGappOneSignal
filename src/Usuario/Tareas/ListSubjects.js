    ...
    return (
      <Container>
          <Content padder>
            <Card>
                <CardItem header>
                    <Text style={{fontSize:35, fontWeight:"bold"}}>English Subjects</Text>
                </CardItem>
              <List dataArray={this.state.dataSource}
                renderRow={(rowData) => {
                    return(
                        <CardItem button onPress={() => this.props.navigation.navigate("Tareas",{idMat:rowData.id,name:rowData.name+" Homeworks", idgrupo:this.state.grupo})}>
